...
/// Add/edit sheet for a per-secret policy rule.
struct RuleEditorSheet: View {
    let existing: SecretRule?
    ...
    @State private var rateLimit = "5/hour"
    @State private var ruleDescription = ""

    private var valid: Bool {
        !name.trimmingCharacters(in: .whitespaces).isEmpty
            && !scope.trimmingCharacters(in: .whitespaces).isEmpty
            && PolicyEngine.rateLimitParse(rateLimit) != nil
    }

    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text(existing == nil ? "Add Secret Rule" : "Edit Secret Rule")
                .font(.kelid(17, .bold))
                .frame(maxWidth: .infinity)

            field("Secret name", text: $name, prompt: "DB_URL", disabled: existing != nil)
            field("Scope", text: $scope, prompt: "database")

            Text("Sensitivity tier")
                .font(.kelid(12, .medium))
                .foregroundStyle(.secondary)
            ...
            field("Rate limit (per secret, any caller)", text: $rateLimit, prompt: "5/hour")
            if PolicyEngine.rateLimitParse(rateLimit) == nil {
                PaneStatus(kind: .error, message: "Format: count/unit — e.g. 5/hour, 20/day, 10/min.")
            }

            Toggle(isOn: $requireReason) {
                Text("Require a reason even for low tier").font(.kelid(12, .medium))
            }
            .toggleStyle(.checkbox)

            field("Purpose (given to the guardian)", text: $ruleDescription, prompt: "production Postgres connection string")
            ...
            HStack {
                Button("Cancel") { dismiss() }
                    ...
                    .buttonBorderShape(.capsule)
                Spacer()
                Button(existing == nil ? "Add Rule" : "Save") {
                    save()
                }
                .buttonStyle(.glassProminent)
                .buttonBorderShape(.capsule)
    ...
    private func field(_ label: String, text: Binding<String>, prompt: String, disabled: Bool = false) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(label)
                .font(.kelid(12, .medium))
                .foregroundStyle(.secondary)
    ...
    private func save() {
        if var updated = existing {
            updated.scope = scope.trimmingCharacters(in: .whitespaces)
            updated.tier = tier
            updated.requireReason = requireReason
            updated.rateLimit = rateLimit.trimmingCharacters(in: .whitespaces)
            updated.ruleDescription = ruleDescription
            store.updateRule(updated)
        } else {
            store.addRule(SecretRule(
                name: name.trimmingCharacters(in: .whitespaces),
                scope: scope.trimmingCharacters(in: .whitespaces),
                tier: tier,
                requireReason: requireReason,
                ruleDescription: ruleDescription,
                rateLimit: rateLimit.trimmingCharacters(in: .whitespaces)
            ))
        }
        dismiss()
    }
